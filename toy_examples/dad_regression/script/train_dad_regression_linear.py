...
class NonlinearRegressionExperiment(nn.Module):
    ...
    def __init__(
        ...
        if beta_loc is None:
            beta_loc = torch.tensor([0.0, 0.0])  # Prior mean
        if beta_scale is None:
            beta_scale = torch.tensor([1.0, 1.0])  # Prior std dev
    ...
    def model(self):
        ...
        # Sample unknown parameters β₀, β₁, β₂
...
def train_dad(
    ...
    model = NonlinearRegressionExperiment(
        ...
        beta_loc=torch.tensor([-0.5, 0.5]),  # Prior mean -0.5, 0.5
        beta_scale=torch.tensor([1.0, 1.0]),  # Prior std dev
        noise_scale=0.1,
        T=T,
    )
    ...
    best_loss = float('inf')
    patience = 200  # How many steps to wait after no improvement
    patience_counter = 0
    ...
    # Evaluate learned design strategy
    print("Storing model to MlFlow... ", end="")
    mlflow.pytorch.log_model(model.cpu(), "model")
    ml_info = mlflow.active_run().info
...
if __name__ == "__main__":
    ...
    parser.add_argument("--num-steps", default=500, type=int) #500
    parser.add_argument("--num-inner-samples", default=1000, type=int) # 100
    parser.add_argument("--seed", default=-1, type=int) #-1
    parser.add_argument("--num-outer-samples", default=2000, type=int) #200
    parser.add_argument("--lr", default=0.001, type=float)
    parser.add_argument("--gamma", default=0.95, type=float)
    parser.add_argument("--hidden-dim", default=16, type=int)
    parser.add_argument("--encoding-dim", default=4, type=int)
    ...
    parser.add_argument(
        "--mlflow-experiment-name", default="dad_regression_linear_newMethod_mmd", type=str
    )
    parser.add_argument(
