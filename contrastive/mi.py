...
class PriorContrastiveEstimation(MutualInformation):
    ...
    def add_jitter_to_prior(self, model):
        if hasattr(model, "theta_prior") and isinstance(model.theta_prior, dist.MultivariateNormal):
            eps = 1e-3
            cov = model.theta_prior.covariance_matrix
            cov_jittered = cov + eps * torch.eye(cov.shape[-1], device=cov.device)
            model.theta_prior = dist.MultivariateNormal(model.theta_prior.loc, cov_jittered).to_event(1)
    ...
    def get_contrastive_rollout(
        ...
        # self.add_jitter_to_prior(model) 
    ...
    def differentiable_loss(self, model, *args, **kwargs):
        ...
        design = self.extract_designs_from_trace(primary_trace)

        loss += 0.01*self.range_regularization(design)

        warn_if_nan(loss, "loss")
        return loss

    def extract_designs_from_trace(self, trace):
        return torch.cat(
            [
                node["value"]
                for name, node in trace.nodes.items()
