...
class SetEquivariantDesignNetwork(nn.Module):
    ...
    def lazy(self, *design_obs_pairs):
        def delayed_function():
            return self.forward(*design_obs_pairs) # out put the design
    ...
    def forward(self, *design_obs_pairs):
        if len(design_obs_pairs) == 0:
            sum_encoding = self.empty_value.new_zeros(self.encoder.encoding_dim).to(self.device)

        else:
            sum_encoding = sum(
                self.encoder(
                xi=design.to(self.device), 
                y=obs.to(self.device), 
                t=[idx + 1]
                )
